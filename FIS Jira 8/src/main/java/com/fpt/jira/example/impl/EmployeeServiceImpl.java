...
if(to !=null) {
	toDate=sdf.format(to);
		...
		SearchEmployeeResult searchEmployeeResult = employeeRepository.searchEmployee(id, name, email, account, fromDate, toDate,recordsPerPage, page);
		
		StringBuilder employeeIdList = new StringBuilder();
		List<EmployeeEntityDto> employeeEntityDtoList =searchEmployeeResult.getEmployeeEntityDtoList();
		for(int i=0;i<employeeEntityDtoList.size();i++) {
		employeeIdList.append("'" + employeeEntityDtoList.get(i).getId() + "',");
		}
		...
		try {
			...
			queryString.append(" SELECT o.id AS USER_ID"+ ", c.summary "+", d.datevalue AS STARTDATE"+", c.duedate"+", e.numbervalue AS COMPLETE");
			queryString.append(" FROM " + "cwd_user o");
			queryString.append(" LEFT JOIN " + "jiraissue c" + " ON o.lower_user_name= c.assignee");
			...
			queryString.append(" WHERE o.id in (" + employeeIdList.substring(0, employeeIdList.length() - 1) + ")");				
			...
			if(toDate != null && !toDate.equals("")) {
				...
				for(int i=0;i<employeeEntityDtoList.size();i++) {
					...
					for(int j=0; j <taskEntityDtoList.size();j++) {
						if(taskEntityDtoList.get(j).getId()==employeeEntityDtoList.get(i).getId()) {
					...
					employeeEntityDtoList.get(i).setPunctualtask(punctual_task);
					employeeEntityDtoList.get(i).setEarlytask(early_task);
					employeeEntityDtoList.get(i).setDelayedtask(delayed_task);
					if(employeeEntityDtoList.get(i).getDelayedtask()==0) {
						employeeEntityDtoList.get(i).setComment("Very good");
					}
					else if(employeeEntityDtoList.get(i).getDelayedtask()!=0&&employeeEntityDtoList.get(i).getEarlytask()>employeeEntityDtoList.get(i).getDelayedtask()) {
						employeeEntityDtoList.get(i).setComment("Good");
					}
					else {
						employeeEntityDtoList.get(i).setComment("Bad");
					}
			}
...
//			public EmployeeEntityDto getEmployeeDto(List<EmployeeEntityDto> employeeEntityDtoList, EmployeeEntityDto employeeEntityDto) {
//				for (int i = 0; i < employeeEntityDtoList.size(); i++) {
//					if(employeeEntityDtoList.get(i).getId() == employeeEntityDto.getId()) {
//						return employeeEntityDtoList.get(i);
//					}
//				}
//				
//				return null;
